    ...
    <main className="glass-panel right-panel animate-fade-in" style={{ animationDelay: '0.2s' }}>
      <div style={{ padding: '2.5rem', flex: 1, overflowY: 'auto' }}>
        <header style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', flexWrap: 'wrap', gap: '1rem', marginBottom: '3rem' }}>
          <div>
            <h1 className="heading" style={{ fontSize: '2.5rem', letterSpacing: '-0.03em' }}>Welcome back.</h1>
            <p className="text-muted" style={{ fontSize: '1.125rem' }}>Here is what's happening with your projects today.</p>
          </div>
          <button style={{
          ...
          >
            <span style={{ fontSize: '1.25rem', lineHeight: 1 }}>+</span> New Project
          </button>
        </header>
        ...
        <div style={{
          ...
          {[
            { title: 'Total Revenue', value: '$24,589', trend: '+12.5%', color: '#10b981' },
            { title: 'Active Users', value: '1,245', trend: '+5.2%', color: '#6366f1' },
            { title: 'Server Load', value: '24%', trend: '-2.1%', color: '#f43f5e' }
          ].map((stat, idx) => (
            <div key={idx} style={{
        ...
        <div style={{ background: 'rgba(0,0,0,0.2)', borderRadius: '1.25rem', padding: '2rem', border: '1px solid var(--panel-border)', minHeight: '300px' }}>
          <h2 className="heading" style={{ fontSize: '1.25rem', marginBottom: '1.5rem' }}>Recent Activity</h2>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
            {[1, 2, 3, 4].map((item) => (
              <div key={item} style={{ display: 'flex', alignItems: 'center', gap: '1rem', padding: '1rem', background: 'rgba(255, 255, 255, 0.02)', borderRadius: '0.75rem', transition: 'var(--transition-fast)' }}
              onMouseOver={(e) => e.currentTarget.style.background = 'rgba(255, 255, 255, 0.05)'}
              onMouseOut={(e) => e.currentTarget.style.background = 'rgba(255, 255, 255, 0.02)'}
              >
                <div style={{ width: '40px', height: '40px', borderRadius: '50%', background: 'rgba(255,255,255,0.1)' }} />
                <div style={{ flex: 1 }}>
                  <h4 style={{ margin: 0, fontWeight: 500 }}>System Update Completes</h4>
                  <p className="text-muted" style={{ margin: 0, fontSize: '0.875rem' }}>Automated task finished at {10 + item}:00 AM</p>
                </div>
                <span className="text-muted" style={{ fontSize: '0.875rem' }}>{item}h ago</span>
              </div>
            ))}
          </div>
        </div>
